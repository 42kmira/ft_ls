...
/*   Updated: 2019/11/18 15:52:17 by kmira            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_tree(t_inode *root, t_h_output *h_output,
						t_inode_type mask)
{
	if (root == NULL)
		return ;
	if (root->left != NULL)
		print_tree(root->left, h_output, mask);
	if (root->type & mask)
	{
	...
	if (root->right != NULL)
		print_tree(root->right, h_output, mask);
}
